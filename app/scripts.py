...
def recently_added_posters(webhooktitle):
    ...
    def restore_from_tmdb():
        ...
        try:
            ...
            logger.warning(
                "RESTORE: " + i.title + " This poster could not be found on TheMoviedb"
            )
    ...
    def decision_tree(tmp_poster):
        ...
        if config[0][37] == 1:
            newdir = (
                os.path.dirname(
                    re.sub(config[0][38], "/films", i.media[0].parts[0].file)
                )
                + "/"
            )
        else:
            path = config[0][38]
            newdir = (
                os.path.dirname(re.sub(config[0][5], path, i.media[0].parts[0].file))
                + "/"
            )
        ...
        if (audio_banner == False and config[0][35] == 1) or (
            hdr_banner == False and config[0][15] == 1
        ):
            scanned = scan_files()
            audio = scanned[0]
        ...
        try:
            if img.status_code == 200:
                ...
                logger.warning(
                    "4k Posters: "
                    + films.title
                    + "cannot find the poster for this film"
                )
        except OSError as e:
            logger.error(e)
    ...
    def search_lib():
        ...
        for i in films.search(title=webhooktitle):
            i.title = (
                unicodedata.normalize("NFD", i.title)
                .encode("ascii", "ignore")
                .decode("utf8")
            )
            logger.info(i.title)
...
def tv4kposter():
    ...
    def get_TVposter():
        newdir = (
            os.path.dirname(re.sub(config[0][5], "/films", i.media[0].parts[0].file))
            + "/"
        )
        backup = os.path.exists(newdir + "poster_bak.png")
        imgurl = i.posterUrl
        ...
        if img.status_code == 200:
                ...
                if backup == True:
                    ...
                    if b_hash - b_hash1 < cutoff:
                        ...
                        try:
                            ...
                            if hash0 - hash1 < cutoff:
                                logger.info(
                                    "4k Posters: Poster has Mini 4k Banner, Skipping Backup"
                                )
                            else:
                                logger.info(
                                    "4k Posters: New poster detected, creating new Backup"
                                )
                                os.remove(poster)
                                logger.info(
                                    "4k Posters: Check passed, creating a backup file"
                                )
            ...
            logger.info(
                "4k Posters: " + i.title + " cannot find the poster for this Episode"
            )
            ...
            for i in tv.searchEpisodes(resolution="4k"):
                try:
                    ...
                    logger.info(
                        "4k Posters: "
                        + tv.title
                        + " The 4k poster for this episode could not be created"
                    )
                    logger.info(
                        "This is likely because poster backups are enabled and the script can't find or doesn't have access to your backup location"
                    )
        ...
        for i in films.search():
            ...
            try:
                ...
                logger.error(
                    "3D Posters: "
                    + films.title
                    + " The 3D poster for this film could not be created."
                )
                continue
...
def restore_posters():
        ...
        def restore_tmdb():
            ...
            try:
                ...
                logger.info(
                    "RESTORE: "
                    + i.title
                    + " This poster could not be found on TheMoviedb"
                )
                pass
        ...
        for i in films.search():
            ...
            if backup == True:
                ...
                try:
                    ...
                    pass
                try:
                    hdr_version = str.lower(
                        x["media"]["track"][1]["HDR_Format_Commercial"]
                    )
                except KeyError:
                    try:
                        hdr_version = str.lower(
                            x["media"]["track"][1]["Format_Commercial_IfAny"]
                        )
                    except KeyError:
                        try:
                            hdr_version = str.lower(
                                x["media"]["track"][1]["HDR_Format"]
                            )
                        except KeyError:
                            hdr_version = "Unknown"
    ...
    def check_connection():
        try:
            ...
            logger.error(e)
            logger.error(
                "Cannot connect to your plex server. Please double check your config is correct."
            )
        else:
            continue_restore()
        ...
        if config[0][20] == 1:
                ...
                if len(resolutions) < 2 and "4k" in resolutions:
                    if config[0][21] == 0:
                        ...
                        logger.info(
                            "Hide-4K: "
                            + movie.title
                            + " has only 4k avaialble, setting untranscodable"
                        )
                    elif config[0][21] == 1:
                        logger.info(
                            "Hide-4K: Sending " + movie.title + " to be transcoded"
                        )
            ...
            for movie in b:
                ...
                if len(resolutions) > 1 and "4k" in resolutions:
                    ...
                    logger.info(
                        "Hide-4K: " + movie.title + " removing untranscodable label"
                    )
    ...
    def popular():
        ...
        for i in range(3):
            ...
            popular = discover.discover_movies(
                {"page": i, "with_original_language": "en"}
            )
            for p in popular:
                for p in popular:
                    title = p.title
    ...
    def recommended():
        if config[0][33] != "":
            try:
                ...
                i = tautulli_api.get_home_stats(
                    stats_type="plays", stat_id="top_movies"
                )
                x = json.dumps(
                    tautulli_api.get_home_stats(
                        stats_type="plays", stat_id="top_movies"
                    )
                )
                i = json.loads(x)
                ...
                rm_collection = [
                    m.title for m in films.search(collection="Recommended")
                ]
